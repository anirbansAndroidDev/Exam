...
public class MonthVsMarks extends Activity {

	private MonthVsMarks CurContext;
	...
	public class AsyncExamDetails extends AsyncTask<String, Void, Boolean> {
		...
		protected void onPreExecute() {
			...
			/*
			 * TableLayout llFeeParent = (TableLayout)
			 * findViewById(R.id.tblFeeLayout); llFeeParent.removeAllViews();
			 * 
			 * LayoutInflater factory = LayoutInflater.from(CurContext);
			 * 
			 * // //ROW
			 * 
			 * try { View FeeHdr = factory.inflate(R.layout.feehdr, null);
			 * TableRow llFeeHdr = (TableRow) FeeHdr
			 * .findViewById(R.id.single_course_row);
			 * 
			 * llFeeParent.addView(llFeeHdr);
			 * 
			 * } catch (Exception ex) {
			 * 
			 * }
	...
	public void PlotSubjectVsResult() {
		...
		LinearLayout llChartHolder = (LinearLayout) findViewById(R.id.llChartHolderMonthVsMarks);
						...
						for (int SubCnt = 0; SubCnt < SubjectCount; SubCnt++) {
							...
							if (SubjectBlock != null) {
								...
								if (examDtl.getSubjectID() > 0) {
									// lstSubjectExamDetails.add(examDtl);
