...
public class ClassExersiseActivity extends Activity implements ViewFactory {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.class_exercise);

		CurContext = this;
		CurContextNew = this.getParent();
	...
	public void ShowAddEditDlg(boolean isAdd, ScheduleInfo schedule,
		...
		adEditSchedule = new AlertDialog.Builder(CurContextNew);
	...
	private void PracticeExamDownloader() {
		...
		pgExam = new ProgressDialog(this.getParent());
		...
		adStartExam = null;
		adStartExam = new Builder(this.getParent());
	...
	public class AsyncQuestionDownloader extends AsyncTask<String, TaskStatusMsg, Integer> {
		...
		{
			//TaskStatusMsg infoUpload = mExamSyncService.AnswerUpload(StudentInfo, (ExamActivity) CurContextNew);
		...
		protected void onProgressUpdate(TaskStatusMsg... values) 
		{
			...
			//Toast.makeText(CurContext, values[0].getMessage(),Toast.LENGTH_LONG).show();
		}
	...
	Handler mProgressHandlerQuestion = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			AlertDialog.Builder dlgMsgbuilder = new AlertDialog.Builder(CurContextNew);
		...
		.setMessage(
				...
				.setNegativeButton("Yes",new DialogInterface.OnClickListener() 
				{
					public void onClick(DialogInterface dialog, int id) {
						...
						{
							Toast.makeText(CurContext,"Incorrect Data: application requires syncing",Toast.LENGTH_SHORT).show();
						}
					}
				});
